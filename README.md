...
## r29 dependency-driven validation liveness

r29 keeps q2/deterministic validation unchanged and removes fixed-interval re-audit for data dependencies.

- `ABILITY_LINEAGE_PENDING` / `ABILITY_PREVIOUS_PENDING` register an ability-prefix waiter instead of a 60ms poll timer.
- advancing `confirmedAbilitySeq` by certificate finalization or bootstrap checkpoint wakes only commands whose required prefix is now available.
- `SIMULATION_REF_PENDING` sleeps until the exact historical simulation sequence is installed.
- unchanged validator receipts are not retransmitted; the signaling server also suppresses identical per-validator progress updates.
- `TICK_AHEAD` and `RESPAWN_EARLY` remain time-based retries because time itself is their dependency.
