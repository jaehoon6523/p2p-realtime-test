...
function handleVerificationReceipt(room, session, message) {
  ...
  const previousReceipt=bucket.receipts.get(session.peerId);
  if(previousReceipt&&previousReceipt.decision===nextReceipt.decision&&previousReceipt.resultCode===nextReceipt.resultCode&&previousReceipt.computedHash===nextReceipt.computedHash&&previousReceipt.evidenceHash===nextReceipt.evidenceHash) return;
  bucket.receipts.set(session.peerId, nextReceipt);
