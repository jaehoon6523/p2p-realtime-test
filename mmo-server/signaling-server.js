...
function computePolicies(room) {
  ...
  for (const [peerId, rec] of room.peers) {
    ...
    const changed = !old || !arraysEqual(old.topologyPeers, topologyPeers) || !arraysEqual(old.simulationPeers, simulationPeers) || !arraysEqual(old.directPeers, directPeers) || !arraysEqual(old.validatorIds, validatorIds);
    const topologyEpoch = changed ? (rec.topologyEpoch || 0) + 1 : old.topologyEpoch;
    const assignmentId = changed ? stableHash({ room: room.channelId, peerId, topologyEpoch, topologyPeers, simulationPeers, validatorIds, rulesetRevision: RULESET_REVISION }) : old.assignmentId;
    const policy = { peerId, topologyEpoch, assignmentId, topologyPeers, simulationPeers, directPeers, validatorIds, quorum: validatorIds.length ? Math.floor(validatorIds.length / 2) + 1 : 0 };
...
wss.on('connection', socket => {
  ...
  socket.on('message', raw => {
    ...
    if (message.type === 'join') {
      ...
      const rec = { socket, aoiRadius: clampNumber(message.aoiRadius, DEFAULT_AOI, MIN_AOI, MAX_AOI), presence: null, topologyEpoch: previous?.topologyEpoch || 0, policy: null };
