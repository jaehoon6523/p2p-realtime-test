...
class BotPeer {
  ...
  async connectSignal(){
    await new Promise((resolve,reject)=>{
      ...
      ws.on('open',()=>{
        ws.send(JSON.stringify({type:'join',channelId:this.room,peerId:this.id,transport:'webrtc',signalProtocol:SIGNAL_PROTOCOL,rulesetRevision:RULESET_REVISION,aoiRadius:AOI_RADIUS}));
      });
      ws.on('message',data=>{
  ...
  async handleSignal(message){
    ...
    if(message.type==='offer') await this.createPeer(message.from,false,message.sdp);
      ...
      if(p?.pc){ p.iceReceived=(p.iceReceived||0)+1; this.log('ice rx',message.from,p.iceReceived); if(p.pc.remoteDescription) await p.pc.addIceCandidate(message.candidate); else p.pendingIce.push(message.candidate); }
  ...
  bindPcEvents(pc,remoteId,entry){
    entry.iceSent=0; entry.iceReceived=0;
    const emitCandidate=candidate=>{
      if(!candidate) return;
      const payload=typeof candidate.toJSON==='function'?candidate.toJSON():candidate;
      entry.iceSent++;
      this.log('ice tx',remoteId,payload?.candidate||payload?.foundation||entry.iceSent);
      this.sendSignal({type:'ice',to:remoteId,candidate:payload});
    };
    const onBrowserIce=ev=>emitCandidate(ev?.candidate);
    const onState=()=>{
      const state=pc.connectionState||pc.iceConnectionState||'unknown';
      this.log('pc state',remoteId,state,`ice tx=${entry.iceSent} rx=${entry.iceReceived}`);
      if(['failed','closed'].includes(state)) this.removePeer(remoteId,`pc ${state}`);
    };
    const onDc=ev=>{ const dc=ev?.channel||ev; if(dc) this.bindDataChannel(remoteId,entry,dc); };

    // werift native Event API. Official trickle-ICE examples use onIceCandidate.subscribe().
    pc.onIceCandidate?.subscribe?.(emitCandidate);
    ...
    // Newer werift releases are progressively W3C-compatible. Register DOM-style
    // listeners as a fallback, but de-duplicate identical ICE candidates below.
    if(typeof pc.addEventListener==='function'){
      pc.addEventListener('icecandidate',onBrowserIce);
  ...
  async createPeer(remoteId,isOfferer,remoteSdp){
    ...
    const {RTCPeerConnection}=this.WebRTC; const pc=new RTCPeerConnection(RTC_CONFIG);
