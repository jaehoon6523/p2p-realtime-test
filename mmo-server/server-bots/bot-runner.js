...
const BOT_TRANSPORT=(process.env.BOT_TRANSPORT||'ws').toLowerCase();
const { BotPeer } = BOT_TRANSPORT==='webrtc' ? require('./bot-peer') : require('./bot-peer-ws');
...
async function main(){
  console.log(`[bot-runner:v5] JOIN phase room=${ROOM_ID} bots=${BOT_COUNT} transport=${BOT_TRANSPORT} signal=${SIGNAL_URL}`);
  for(let i=0;i<BOT_COUNT;i++){
    ...
    await bot.start();
    console.log(`[bot-runner:v5] joined ${i+1}/${BOT_COUNT} ${id} assignment=${bot.protocol.selfPolicy?.assignmentId||'-'} direct=${bot.desiredDirectPeers.size}`);
    if(i+1<BOT_COUNT) await wait(JOIN_GAP_MS);
  }
  console.log('[bot-runner:v5] SETTLE phase');
  ...
  const ready=bots.filter(b=>b.isStable()).length;
  console.log(`[bot-runner:v5] RUN phase topology ready=${ready}/${bots.length}`);
  for(const bot of bots) bot.enableAI();
}
async function shutdown(){ console.log('\n[bot-runner:v5] stopping'); await Promise.allSettled(bots.map(b=>b.stop())); process.exit(0); }
process.on('SIGINT',shutdown); process.on('SIGTERM',shutdown);
main().catch(error=>{ console.error('[bot-runner:v5] fatal:',error.stack||error.message); process.exit(1); });
