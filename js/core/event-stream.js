...
function removePending(command){
    ...
    if(typeof clearVerificationTracking==='function') clearVerificationTracking(command);
...
function ingestCommand(command,remote,{reentry=false}={}){
    ...
    if(commandStream(command)==='event'){
        ...
        if(ref.status==='pending'){
            deferCommand(command,remote,'SIMULATION_REF_PENDING',`waiting for simulation seq=${command.simulationRef.sequence}`);
            return;
        }
