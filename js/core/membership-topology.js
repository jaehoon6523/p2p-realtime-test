...
function refreshMembership(reason=''){
    membershipRevision++;
    document.getElementById('membershipVersion').textContent=`${serverPeerCount} room / ${directOpenPeerIds().length} direct`;
    document.getElementById('epochStat').textContent=serverMembershipEpoch||'-';
    if(reason) log('t-sys',`membership room=${serverPeerCount} direct=${directOpenPeerIds().length} epoch=${serverMembershipEpoch||'-'} root=${serverMembershipRoot||'-'} reason=${reason}`);
}
