...
const RULESET_REVISION = 'pssf-v13-r29';
...
// Dependency-driven audit liveness. Ability waiters sleep until the exact prerequisite
// terminal prefix advances; they do not poll on a fixed timer.
const abilityAuditWaiters = new Map(); // playerId -> Map(requiredAbilitySeq -> Set(commandId))
const abilityAuditWaiterByCommand = new Map(); // commandId -> {playerId,requiredSeq}
const auditWakeQueued = new Set(); // commandId, microtask de-duplication
...
function rememberSimulationState(playerId,state){
    ...
    // Historical simulation references are a data dependency, not a clock dependency.
    // Retry only after the requested sequence actually becomes available.
    if(typeof wakeSimulationReferenceDependencies==='function') wakeSimulationReferenceDependencies(playerId,state.sequence);
}
