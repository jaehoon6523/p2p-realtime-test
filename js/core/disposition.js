...
function importAbilityCheckpoint(playerId,checkpoint){
    ...
    if(typeof wakeAbilityAuditDependencies==='function') wakeAbilityAuditDependencies(playerId);
...
const deferredDependencyWakeQueued=new Set();
function scheduleDeferredDependencyWake(commandId){
    if(!commandId||deferredDependencyWakeQueued.has(commandId)) return false;
    deferredDependencyWakeQueued.add(commandId);
    const wake=()=>{
        deferredDependencyWakeQueued.delete(commandId);
        const item=deferredCommands.get(commandId);
        if(!item) return;
        retryDeferredCommand(commandId);
    };
    if(typeof queueMicrotask==='function') queueMicrotask(wake); else setTimeout(wake,0);
    return true;
}
function wakeSimulationReferenceDependencies(playerId,availableSequence){
    if(!playerId||!Number.isSafeInteger(availableSequence)) return 0;
    let count=0;
    for(const [commandId,item] of deferredCommands){
        if(item.command.playerId!==playerId||item.code!=='SIMULATION_REF_PENDING') continue;
        if(item.command.simulationRef?.sequence!==availableSequence) continue;
        if(scheduleDeferredDependencyWake(commandId)) count++;
    }
    return count;
}
