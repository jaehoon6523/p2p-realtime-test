...
function startMove(playerId,fromX,fromY,toX,toY){
    ...
    moveState[myId]={startX,startY,targetX:toX,targetY:toY,dirX:dx/distance,dirY:dy/distance,distance,startTime:now,profile,hardStopAt:now+Math.max(MOVE_MAX_DURATION,(profile.totalTime+1)*1000),sampleX:startX,sampleY:startY,lastX:startX,lastY:startY};
...
function tickMovement(){
    ...
    if(evaluated.finished){
        ...
        for(let i=0;i<8;i++){ const tail=getPredictedTail(myId); if(!tail) break; const dx=movement.targetX-tail.x,dy=movement.targetY-tail.y; if(Math.hypot(dx,dy)<=0.01) break; executeLocal(makeMoveCommand(dx,dy)); }
