...
```text
[p2p-arena-signaling] protocol=5 ruleset=pssf-v13-r28 base<=5 sim<=16 listening ws://0.0.0.0:<PORT>
```
...
- Expected build/ruleset: `pssf-v13-r28`
- Demo HTML script URLs are pinned with `?v=pssf-v13-r28`.
- Public test URLs should include `&build=pssf-v13-r28` so the HTML document itself also gets a distinct cache key.
- After deploy, reject any browser log whose `local peer boot` line is not `ruleset=pssf-v13-r28`.


After deployment, verify the first boot line reports `ruleset=pssf-v13-r28`. For multiplayer diagnostics, AUTO pages with `autodebug=1` should show `AUDIT_EVAL` and `AUDIT_RECEIPT_TX` when receiving Q/W/E.
