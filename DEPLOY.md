...
```text
[p2p-arena-signaling] protocol=5 ruleset=pssf-v13-r29 base<=5 sim<=16 listening ws://0.0.0.0:<PORT>
```
...
- Expected build/ruleset: `pssf-v13-r29`
- Demo HTML script URLs are pinned with `?v=pssf-v13-r29`.
- Public test URLs should include `&build=pssf-v13-r29` so the HTML document itself also gets a distinct cache key.
- After deploy, reject any browser log whose `local peer boot` line is not `ruleset=pssf-v13-r29`.


After deployment, verify the first boot line reports `ruleset=pssf-v13-r29`. For multiplayer diagnostics, AUTO pages with `autodebug=1` should show `AUDIT_EVAL` and `AUDIT_RECEIPT_TX` when receiving Q/W/E.

## r29 validation-liveness diagnostics
Expected steady-state behavior:
- one `AUDIT_RECEIPT_TX ... ABILITY_LINEAGE_PENDING` per unchanged dependency state, not dozens per second;
- optional `AUDIT_WAIT ... dependency=ability:<peer>:a<N>` in debug mode;
- when ability `a<N>` finalizes/imports, the dependent command should immediately re-audit and emit a changed receipt;
- `SIMULATION_REF_PENDING` should re-run when the requested simulation sequence arrives, not on a fixed polling cadence.
