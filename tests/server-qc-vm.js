...
vm.runInContext(`
 ...
 for(const [name,stream,seq] of cases){ vote('v1',name,stream,seq,name+'-ev',name+'-computed'); vote('v2',name,stream,seq,name+'-ev',name+'-computed'); const cert=sent.map(x=>x.msg).find(m=>m.type==='verification-certificate'&&m.commandId===name); if(!cert||cert.verdict!=='accepted'||cert.quorum!==2||cert.computedHash!==name+'-computed') throw new Error(name+' q2 certificate missing'); }
